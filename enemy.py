...
class Enemy(pygame.sprite.Sprite):
    def __init__(self, x, y, e_type=None):
        ...
        if e_type is not None:
            self.image = self.enemy_state_types[e_type]['default']
        else:
            self.image = self.enemy_state_types[random.choice(list(self.enemy_state_types.keys()))]['default']
    ...
    def load_images(self):
        return {
            'dog': {
                ...
                'collide': scale_img(Path('assets', 'imgs', 'dog_state2.png'), 100),
            },
            'out_cat': {
                'default': scale_img(Path('assets', 'imgs', 'cat_state1.png'), 100),
                'collide': scale_img(Path('assets', 'imgs', 'cat_state2.png'), 100),
            },
            'blond_girl': {
                'default': scale_img(Path('assets', 'imgs', 'girl_type1_w_hotdog.png'), 100),
                'collide': scale_img(Path('assets', 'imgs', 'girl_type1_w_hotdog.png'), 100),
            }
        }
    ...
    def get_allowed_spots(cls):
        return {
            'dog': None,
            'out_cat': [(300, 200), (400, 200), (500, 200)],
            'blond_girl': None
        }
